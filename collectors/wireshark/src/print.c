...
static char* get_data_buf(data_buf_t *cur_b,int size){
    size = size+2;
    if(cur_b->p==NULL){
        cur_b->p = (char*)malloc(size);
        cur_b->len = size;
    }else if(cur_b->len<size){
        free(cur_b->p);
        cur_b->p =(char*)malloc(size);
        cur_b->len = size;
    }

    if(cur_b->p)memset(cur_b->p,0,cur_b->len);
...
static void make_data_full_path(const char* data_dir,char* data_full_path,char* fn_buf,int id){
    char tstr[300]={0};
    time_t rawtime;
    struct tm* timeinfo;
    int dir_len,fn_len;
    ...
    strftime(tstr,299,"/%Y%m%d-%I%M%S",timeinfo);
    dir_len = strlen(data_dir);
    fn_len = strlen(tstr);
    
    p = data_full_path;

    if(data_dir[dir_len-1]=='/'){
        memcpy(p,data_dir,dir_len-1);
        p += (dir_len-1);
    }
    else{
        
        memcpy(p,data_dir,dir_len);
        p += dir_len;
    }

    memcpy(p,tstr,fn_len);

    p+= fn_len;
    
    sprintf(p,"-%d",id);
    sprintf(fn_buf,"%s-%d",tstr,id);
}

static void write_mirror_pipe(FILE *fp,const char* fn){

 char pre[1024]={0};
 sprintf(pre, "%s %s %s %s [%s] \"%s\" %u %s \"%s\" \"%s\" %s \"%s\"",
          "www.knownsec.com",
          "1.1.1.1",
          "client",
          "server",
          "09/Jun/2014:13:20:19 +0800",
          "GET / HTTP/1.1",
          200,
          "11",
          "www", "www","www","wwww");

        fprintf(fp,"%s %s %d %d md5:%s\n",pre,fn,0,0,"ffff");
        fflush(fp);
}
...
static void write_ip_mirro(proto_tree *tree,cJSON *json_root,data_buf_t *db){
	//ipv4_addr *ip_dst,*ip_src;
	
	/*Get ip.src*/
	get_str_value(tree,hf_ip_src,db);	
	cJSON_AddStringToObject(json_root,"Source_IP",db->p);
	/*Get ip.dst*/
	get_str_value(tree,hf_ip_dst,db);	
	cJSON_AddStringToObject(json_root,"Dest_IP",db->p);
	//printf("----------------ip.src=%s,ip.dst=%s\n",ip_src_str,ip_dst_str);
	cJSON_AddStringToObject(json_root,"Country_Name","");
	cJSON_AddStringToObject(json_root,"City_Name","");
  cJSON_AddNumberToObject(json_root,"Region_Code",0);
}

static void write_tcp_mirro(proto_tree *tree,cJSON *json_root){	
	GPtrArray *finfo_array;
	guint32 src_port;
	guint32 dst_port;

	/* Get tcp.src.port */
	finfo_array = proto_find_finfo(tree,hf_tcp_srcport);
	
	if(g_ptr_array_len(finfo_array)<1){
		printf("tcp.srcport===none\n");
		src_port = 0;
	}else{
		src_port = fvalue_get_uinteger(&((field_info*)finfo_array->pdata[0])->value);
		g_ptr_array_free(finfo_array,TRUE);
	}

	/*Get tcp.dstport*/
	finfo_array = proto_find_finfo(tree,hf_tcp_dstport);
	if(g_ptr_array_len(finfo_array)<1){
		printf("tcp.dstport===none\n");
		dst_port = 0;
	}else{
		dst_port = fvalue_get_uinteger(&((field_info*)finfo_array->pdata[0])->value);
		g_ptr_array_free(finfo_array,TRUE);
	}
	
	//printf("------------------tcp.src_port=%d,tcp.dst_port=%d\n",src_port,dst_port);

	//cJSON_AddNumberToObject(json_root,"Src_Port",src_port);
	cJSON_AddNumberToObject(json_root,"Dest_Port",dst_port);
}

static const char* get_http_query_str(char* uri){
	char *pos;
	if(uri==NULL) return NULL;
	pos = strstr(uri,"?");

	if(!pos||pos==(uri+strlen(uri)-1)) return NULL;
	

	return pos+1;
}

static void write_http_mirro(proto_tree *tree,cJSON *json_root,data_buf_t *db){
	cJSON *req_headers,*get_args,*post_args,*res_headers,*alerts;
	char *ret;
  
	req_headers = cJSON_CreateObject();
	get_args = cJSON_CreateObject();
	post_args = cJSON_CreateObject();
	res_headers = cJSON_CreateObject();
	alerts = cJSON_CreateObject();
	
	/*Get request site */
	ret= get_str_value(tree,hf_http_request_full_uri,db);
	cJSON_AddStringToObject(json_root,"Site",ret==NULL?"":ret);
	
    /*Get request method*/
	ret = get_str_value(tree,hf_http_request_method,db); 
	cJSON_AddStringToObject(json_root,"Request_Method",ret==NULL?"":ret);
  
	/*Get request uri */
	ret = get_str_value(tree,hf_http_request_uri,db);
  cJSON_AddStringToObject(json_root,"Request_Uri",ret==NULL?"":ret);
	
  /*Get request proto */
	ret = get_str_value(tree,hf_http_version,db);
	cJSON_AddStringToObject(json_root,"Request_Protocol",ret==NULL?"":ret);

	/*Get request host */
	ret = get_str_value(tree,hf_http_host,db);
	cJSON_AddStringToObject(json_root,"Host",ret==NULL?"":ret);

	/*Get request UserAgent */
	ret = get_str_value(tree,hf_http_user_agent,db);
	cJSON_AddStringToObject(json_root,"User_Agent",ret==NULL?"":ret);

	/*Get request refer  */
	ret = get_str_value(tree,hf_http_referer,db);
	cJSON_AddStringToObject(json_root,"Referer",ret==NULL?"":ret);
	
	/*Get Query string */
	ret = get_http_query_str(get_str_value(tree,hf_http_request_uri,db));
	cJSON_AddStringToObject(json_root,"Query_String",ret==NULL?"":ret);
  
  cJSON_AddNumberToObject(json_root,"Time",time(NULL));
  cJSON_AddStringToObject(json_root,"Webappid","");
  cJSON_AddNumberToObject(json_root,"Bytes_Sent",0);
	cJSON_AddStringToObject(json_root,"Sessionid","");
  cJSON_AddStringToObject(json_root,"Request_Raw","");

	cJSON_AddNumberToObject(json_root,"Is_Attack",0);
	cJSON_AddNumberToObject(json_root,"Is_Bot",0);
	cJSON_AddNumberToObject(json_root,"Blocked",0);
	cJSON_AddNumberToObject(json_root,"MRule_Id",0);
	cJSON_AddStringToObject(json_root,"MRule_Type","");
	cJSON_AddStringToObject(json_root,"MPayload","");
	cJSON_AddNumberToObject(json_root,"MRule_Severity",0);
	cJSON_AddNumberToObject(json_root,"Response_Status",200);
		
	/*Get request connection header */
	ret = get_str_value(tree,hf_http_connection,db);
	if(ret)cJSON_AddStringToObject(req_headers,"Connection",ret);

	/*Get request cookie header */
	ret = get_str_value(tree,hf_http_cookie,db);
	if(ret)cJSON_AddStringToObject(req_headers,"Cookie",ret);

	/*Get request accept header */
	ret = get_str_value(tree,hf_http_accept,db);
	if(ret)cJSON_AddStringToObject(req_headers,"Accept",ret);

	/*Get request accept ecoding header */
	ret = get_str_value(tree,hf_http_accept_encoding,db);
	if(ret)cJSON_AddStringToObject(req_headers,"Accept-Encoding",ret);

	/*Get request accept language header */
	ret = get_str_value(tree,hf_http_accept_language,db);
	if(ret)cJSON_AddStringToObject(req_headers,"Accept-Language",ret);
	
	cJSON_AddItemToObject(json_root,"Request_Headers",req_headers);
	
	cJSON_AddItemToObject(json_root,"Get_Args",get_args);
	cJSON_AddItemToObject(json_root,"Post_Args",post_args);
	cJSON_AddItemToObject(json_root,"Response_Headers",res_headers);
	cJSON_AddItemToObject(json_root,"Alerts",alerts);
    #if 0
	printf("req_method:%s\n",req_method);
...
void proto_tree_write_mirro(epan_dissect_t *edt,const char* data_dir,FILE* fp,int id){
	char *json_str;
	FILE *fh;
  data_buf_t data_b,*data_b_ptr=&data_b;
  data_b_ptr->p = NULL;
  data_b_ptr->len=0;

	cJSON *json_root=cJSON_CreateObject(); 
	char data_full_path[1024]={0};
  char fn_buf[64]={0};
		
    make_data_full_path(data_dir,data_full_path,fn_buf,id);

    fh = fopen(data_full_path,"w+");
	...
	write_ip_mirro(edt->tree,json_root,data_b_ptr);
	write_tcp_mirro(edt->tree,json_root);
	write_http_mirro(edt->tree,json_root,data_b_ptr);
	json_str = cJSON_Print(json_root);
	fprintf(fh,"%s",json_str);
	fclose(fh);
  if(json_root)cJSON_Delete(json_root);
	if(json_str)free(json_str);
  if(data_b_ptr->p) free(data_b_ptr->p); 
    //proto_tree_write_pdml(edt,fh);
	write_mirror_pipe(fp,fn_buf);
}
