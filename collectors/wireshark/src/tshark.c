...
static char mirro_data_dir[1024]; 
static FILE *mirro_output;
...
#define OPTSTRING "2a:" OPTSTRING_A "b:" OPTSTRING_B "c:C:d:De:E:f:F:gG:hH:i:" OPTSTRING_I "K:lLnN:o:O:pPqQr:R:s:S:t:T:J:M:u:vVw:W:xX:y:Y:z:"
...
/** Send All g_log messages to our own handler **/
  ...
  while ((opt = getopt(argc, argv, optstring)) != -1) {
    ...
    case 'T':        /* printing Type */
      ...
      break;
	case 'J':
		memset(mirro_data_dir,0,1024);
		strcpy(mirro_data_dir,optarg);
		break;
	case 'M':
		mirro_output = popen(optarg,"w");
		if(!mirro_output){
...
print_packet(capture_file *cf, epan_dissect_t *edt)
{
  ...
  if (print_details) {
    ...
    case WRITE_MIRROR:
      proto_tree_write_mirro(edt,mirro_data_dir,mirro_output,packet_count+1);
